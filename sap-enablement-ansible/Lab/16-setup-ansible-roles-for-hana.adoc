...
In our demo environment no DNS domain is set and _ansible_fqdn_ is empty, we need to set it to `$GUID.internal`. In Ansible we have the option to access shell environment variables with the lookup function:
